...
public class AppBundlerTask extends Task {
	...
	private final ArrayList<FileSet> classPath = new ArrayList<>();
	private final ArrayList<FileSet> libraryPath = new ArrayList<>();
	private final ArrayList<Option> options = new ArrayList<>();
	private final ArrayList<String> arguments = new ArrayList<>();
	private final ArrayList<String> architectures = new ArrayList<>();
	private final ArrayList<String> registeredProtocols = new ArrayList<>();
	private final ArrayList<BundleDocument> bundleDocuments = new ArrayList<>();
	private File splashImage = null;
	...
	public void addConfiguredRuntime(FileSet runtime) throws BuildException {
		...
		runtime.appendExcludes(new String[] { "bin/", "jre/bin/", "jre/lib/deploy/", "jre/lib/deploy.jar", "jre/lib/javaws.jar", "jre/lib/libdeploy.dylib",
				"jre/lib/libnpjp2.dylib", "jre/lib/plugin.jar", "jre/lib/security/javaws.policy" });
	}
	...
	public void execute() throws BuildException {
		...
		if (splashImage != null) {
			if (!splashImage.exists()) {
				throw new IllegalStateException("Splash image does not exist.");
			}

			if (splashImage.isDirectory()) {
				throw new IllegalStateException("Invalid splash image.");
			}
		}
		...
		try {
			...
			// Copy splash image to Resources folder
			copySplashImage(resourcesDirectory);
	...
	private void copySplashImage(File resourcesDirectory) throws IOException {
		if (splashImage != null) {
			copy(splashImage, new File(resourcesDirectory, splashImage.getName()));
		}
	}
	...
	private void writeInfoPlist(File file) throws IOException {
		...
		try {
			...
			if (splashImage != null) {
				writeString(xout, "-splash:$APP_ROOT/Contents/Resources/" + splashImage.getName());
			}
